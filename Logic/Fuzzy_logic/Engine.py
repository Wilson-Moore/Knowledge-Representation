...
class FuzzyLungDiseaseSystem:
    ...
    def _prepare_inputs(self, patient: Patient) -> Dict[str, float]:
        ...
        print(f"\n[DEBUG] Patient {patient.id}:")
        for fuzzy_name, symptom_name in symptom_mapping.items():
            print(f"  {fuzzy_name}: {inputs[fuzzy_name]} (recherché: {symptom_name})")
    ...
    def _crisp_to_risk_level(self, risk_value: float) -> RiskLevel:
        ...
        # CORRECTION DES SEUILS POUR CORRESPONDRE AU DATASET
        if risk_value < 2:  # Low: 0-3.0
            return RiskLevel.LOW
        elif risk_value < 3:  # Medium: 3.0-6.0
